...
| Published browser timing snapshot | Interactive-browser home navigation observed approximately **2.75 s response start**, **4.45 s DOM content loaded**, and **4.46 s load completion**. The reported initial JS transfer was **138.5 kB** and CSS transfer **21.0 kB** for that cached session. | Supplemental one-session lab signal only; this is not a Lighthouse run, field Core Web Vitals, or an availability SLA. |
