...
| CI/CD pipeline | **PASS / PARTIAL** | `.github/workflows/ci.yml` runs frozen install, production dependency audit, type check, tests, and build on push to `main`, `production-readiness-audit-plan`, and the non-default `production-readiness-verified-*` snapshot pattern, plus pull requests. The workflow pins Node 24-capable `actions/checkout@v6`, `actions/setup-node@v6`, and pnpm 10-compatible `pnpm/action-setup@v6`; the official decision record is in `reports/ci-runtime-upgrade-sources.md`. [GitHub Actions run 32655491665](https://github.com/aliahmed-io/mousa-glass/actions/runs/32655491665) succeeded at safe-branch commit `1c8cb349d5821821b933aba463ddc192126b6ff6`; [run 32669326623](https://github.com/aliahmed-io/mousa-glass/actions/runs/32669326623) passed the prior snapshot; [run 32669529857](https://github.com/aliahmed-io/mousa-glass/actions/runs/32669529857) passed the upgraded action pins without the Node 20 runtime deprecation warning; and [run 32670358214](https://github.com/aliahmed-io/mousa-glass/actions/runs/32670358214) passed the latest verified snapshot commit `a30bdb16c916b6adc1c14f453d3806419332feb6`. A production smoke test and a protected/main-branch integration process remain absent. |
...
| Image optimization | **PASS / PARTIAL** | Shared hero uses managed responsive WebP variants with media-specific preload and noncritical image deferral. The five documented generated staging products use source-specific 480/960 WebP `srcSet` sources on both Home featured cards and Shop cards. The one Shop card that later reproduced an overfetch has documented full-composition 720px and 800px candidates; the cache-busted `9f6faccc` audit again has empty image-delivery evidence. Future merchant media remains intentionally unmapped until approved assets are supplied. |
...
| Published Shop 800px responsive refinement | **PASS for verified scope** | The initial 720px candidate did not meet the Lighthouse mobile selection threshold and the amber-glassware card continued to fetch its 960px source. The documented 800px candidate resolved the reproducible image-delivery finding in cache-busted audit `9f6faccc`: Performance 62, Accessibility 100, Best Practices 92, SEO 61, FCP 3.6 s, LCP 5.8 s, TBT 60 ms, CLS 0.002, and an empty image-delivery evidence array. Initial server response (1.61 s), unused JavaScript (36 KiB), staging crawl blocking, and managed CSP/no-store observations remain separately tracked. |
...
| Automated test suite | **PASS / PARTIAL** | 65 tests across 11 files pass, including commerce, lifecycle, security, request-correlation and diagnostic-log safety, crawler/SEO, storage proxy, health contract, owner-alert, reverse-layout-shift, responsive staging media, explicit Shop landmark, Express 5-compatible SPA fallback, shared high-risk mutation rate-limit coverage with proxy-normalized identity selection, and safe-branch CI trigger/action-pin coverage. UI E2E, load, and true production smoke coverage are missing. |
...
| Health/readiness endpoint | **PASS / PARTIAL** | `GET /healthz` returns HTTP 200 with only stable non-sensitive `{ status, service, timestamp }`; the payload has focused regression coverage. A later published `/api/healthz` recheck through HTTP/1.1, no-cache HTTP/2, and preview all returned the JSON contract and opaque `X-Request-Id`, resolving one transient empty-body capture. Independent uptime monitoring remains unproven. |
