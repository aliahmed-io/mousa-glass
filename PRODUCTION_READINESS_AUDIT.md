...
| Reproducible validation | `pnpm release:check` completed: production dependency audit, `pnpm check`, `pnpm test`, `pnpm build`, and assigned staging-asset verification. Latest suite: **11 test files, 51 tests passed**. It includes lifecycle/restock, protected-storage, request-security, historical-product deletion, administrator archive-path, retention-gate, proof-reference-deletion, crawler/structured-data, health-contract, owner-alert, provider-scoping, and responsive-banner regressions. | Build integrity and automated regression coverage. |
