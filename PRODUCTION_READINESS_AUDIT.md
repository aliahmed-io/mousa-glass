...
| ARIA role | Published Home and Shop reports identified `role="status"` as invalid on the staging `<aside>`. The component now uses valid `aria-live="polite"` and `aria-atomic="true"` attributes; local type, test, build, and visual checks pass. | **Application fix awaiting published re-audit**. |
