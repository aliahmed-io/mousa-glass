...
Local bundle follow-up found no production use of the root `Toaster` or global `TooltipProvider`; tooltip usage is limited to an un-routed component showcase. Removing those providers from `App` reduced the shared entry from **626.00 kB / 188.95 kB gzip** to **547.63 kB / 163.79 kB gzip** in a full local production build. A focused regression now asserts that `App` imports neither provider nor `sonner`, while the only remaining `sonner` use is in the un-routed showcase. Type checks, 48 regression tests, build, and mobile Home/Shop visual checks passed. The responsive-banner published audit records **36 KiB** unused JavaScript from the transferred shared entry; it is not a direct before/after attribution because lab conditions vary. Authenticated administrator interaction testing remains an account-authority gate: the available authenticated browser is correctly non-admin, and no role was changed merely to close that verification item.
