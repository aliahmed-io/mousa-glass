...
import { AlertTriangle, CheckCircle2, Eye, EyeOff, FolderTree, Loader2, Pencil, Plus, RefreshCw, Save, Search, Trash2, TriangleAlert, X } from "lucide-react";
...
function Editor({ category, close }: { category?: any; close: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (form.slug.trim() && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(form.slug.trim())) return setError("اكتب الرابط المختصر بحروف إنجليزية صغيرة وأرقام وشرطات فقط.");
...
export default function AdminCategories() {
  ...
  async function toggleVisibility(category: any) {
    setError("");
    try { await update.mutateAsync({ id: category.id, isActive: !category.isActive }); } catch (caught) { setError(caught instanceof Error ? caught.message : "تعذر تحديث ظهور القسم. حاول مرة أخرى."); }
  }
  return <DashboardLayout><div className="space-y-7">
    ...
    {q.isLoading ? <div className={`${panelClass} p-12 text-center text-sm text-[#f5f0e8]/50`}>جارٍ تحميل الأقسام…</div> : <><div className="space-y-3 md:hidden">{categories.map(category => <article key={category.id} className={`${panelClass} p-4`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="truncate font-black">{category.name}</h2><p dir="ltr" className="mt-1 truncate text-xs text-[#f5f0e8]/45">{category.slug}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs ${category.isActive ? "bg-emerald-400/10 text-emerald-300" : "bg-white/5 text-[#f5f0e8]/45"}`}>{category.isActive ? "ظاهر" : "مخفي"}</span></div><p className="mt-3 min-h-10 text-sm leading-5 text-[#f5f0e8]/55">{category.description || "لا يوجد وصف لهذا القسم."}</p><div className="mt-4 flex items-center justify-between gap-3 border-t border-[#d4af37]/10 pt-3"><p className="text-xs text-[#f5f0e8]/50">ترتيب العرض: <span className="font-bold text-[#d4af37]">{category.sortOrder}</span></p><div className="flex gap-2"><Button size="sm" variant="outline" disabled={update.isPending} aria-label={category.isActive ? `إخفاء قسم ${category.name}` : `إظهار قسم ${category.name}`} className="rounded-lg border-[#d4af37]/30 text-[#d4af37]" onClick={() => void toggleVisibility(category)}>{category.isActive ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}</Button><Button size="sm" variant="outline" aria-label={`تعديل قسم ${category.name}`} className="rounded-lg border-[#d4af37]/30 text-[#d4af37]" onClick={() => { setAdding(false); setEdit(category); }}><Pencil className="h-3.5 w-3.5" /></Button><Button size="sm" variant="ghost" disabled={remove.isPending} aria-label={`حذف قسم ${category.name}`} className="rounded-lg text-rose-300 hover:bg-rose-500/10" onClick={() => void deleteCategory(category)}><Trash2 className="h-3.5 w-3.5" /></Button></div></div></article>)}{categories.length === 0 && <div className={`${panelClass} p-10 text-center text-[#f5f0e8]/50`}><FolderTree className="mx-auto h-8 w-8 text-[#d4af37]/60" /><p className="mt-3 font-bold">{search ? "لا توجد نتائج مطابقة" : "لا توجد أقسام بعد"}</p><p className="mt-2 text-sm">{search ? "جرّب كلمة بحث مختلفة." : "أضف أول قسم لبدء تنظيم المنتجات."}</p></div>}</div><div className={`${panelClass} hidden overflow-hidden md:block`}><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-right text-sm"><thead className="bg-black/35 text-xs text-[#d4af37]/75"><tr><th className="p-4">القسم</th><th className="p-4">الرابط</th><th className="p-4">الوصف</th><th className="p-4">الترتيب</th><th className="p-4">الظهور</th><th className="p-4">الإجراءات</th></tr></thead><tbody>{categories.map(category => <tr key={category.id} className="border-t border-[#d4af37]/10 transition hover:bg-white/[.025]"><td className="p-4 font-bold">{category.name}</td><td dir="ltr" className="p-4 text-[#f5f0e8]/45">{category.slug}</td><td className="max-w-64 truncate p-4 text-[#f5f0e8]/55">{category.description || "—"}</td><td className="p-4">{category.sortOrder}</td><td className="p-4"><span className={`rounded-full px-2.5 py-1 text-xs ${category.isActive ? "bg-emerald-400/10 text-emerald-300" : "bg-white/5 text-[#f5f0e8]/45"}`}>{category.isActive ? "ظاهر" : "مخفي"}</span></td><td className="p-4"><div className="flex gap-2"><Button size="sm" variant="outline" disabled={update.isPending} aria-label={category.isActive ? `إخفاء قسم ${category.name}` : `إظهار قسم ${category.name}`} className="rounded-lg border-[#d4af37]/30 text-[#d4af37]" onClick={() => void toggleVisibility(category)}>{category.isActive ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}</Button><Button size="sm" variant="outline" aria-label={`تعديل قسم ${category.name}`} className="rounded-lg border-[#d4af37]/30 text-[#d4af37]" onClick={() => { setAdding(false); setEdit(category); }}><Pencil className="h-3.5 w-3.5" /></Button><Button size="sm" variant="ghost" disabled={remove.isPending} aria-label={`حذف قسم ${category.name}`} className="rounded-lg text-rose-300 hover:bg-rose-500/10" onClick={() => void deleteCategory(category)}><Trash2 className="h-3.5 w-3.5" /></Button></div></td></tr>)}{categories.length === 0 && <tr><td colSpan={6} className="p-10"><div className="text-center text-[#f5f0e8]/50"><FolderTree className="mx-auto h-8 w-8 text-[#d4af37]/60" /><p className="mt-3 font-bold">{search ? "لا توجد نتائج مطابقة" : "لا توجد أقسام بعد"}</p><p className="mt-2 text-sm">{search ? "جرّب كلمة بحث مختلفة." : "أضف أول قسم لبدء تنظيم المنتجات."}</p></div></td></tr>}</tbody></table></div></div></>}
  </div></DashboardLayout>;
}
