...
function ShopCardImage({ url, alt, prioritize }: { url: string; alt: string; prioritize: boolean }) {
  const [status, setStatus] = useState<ImageStatus>("loading");
  const isLoaded = status === "loaded";
  const message = status === "loading" ? "جارٍ تحميل صورة المنتج" : status === "error" ? "تعذر عرض صورة المنتج" : "";

  return <div className="relative h-full w-full overflow-hidden bg-[radial-gradient(circle_at_50%_70%,rgba(212,175,55,.18),transparent_42%),linear-gradient(135deg,#12131a,#050505)]" aria-busy={!isLoaded}>
    <img
      {...responsiveStagingProductImage(url)}
      alt={alt}
      ...
      className={`h-full w-full object-cover transition-[opacity,transform] duration-500 group-hover:scale-105 ${isLoaded ? "opacity-100" : "opacity-0"}`}
    />
    {!isLoaded && <div className="pointer-events-none absolute inset-0 grid place-items-center p-4 text-center" aria-hidden="true"><span className="rounded-full border border-[#d4af37]/25 bg-black/45 px-3 py-1.5 text-xs font-bold text-[#f5f0e8]/70">{message}</span></div>}
    {!isLoaded && <span className="sr-only" aria-live="polite">{message}</span>}
  </div>;
}
...
export default function Shop() {
  ...
  const [sort, setSort] = useState<CatalogSort>(() => isCatalogSort(params.get("sort")) ? params.get("sort") as CatalogSort : "featured");
  ...
  const resetCatalog = () => { setSearch(""); navigate("/shop"); };
  ...
  return <StoreLayout><main id="main-content" role="main" className="min-h-[70vh] bg-[#08090d] py-12 sm:py-16"><div className="container"><div className="max-w-2xl"><p className="text-sm font-bold tracking-[.22em] text-[#d4af37]/70">متجر موسى</p><h1 className="mt-3 text-4xl font-black text-[#f5f0e8] md:text-5xl"><span className="text-gold-gradient">كل إكسسوارات الزجاج</span></h1><p className="mt-4 leading-7 text-[#f5f0e8]/60">اختر القطع المناسبة لمشروعك وأضفها إلى السلة لإتمام طلبك بسهولة.</p></div>
    ...
    <div className="mt-5 grid min-h-44 gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3 xl:grid-cols-4">{isLoading && <div className="col-span-full min-h-44 animate-pulse rounded-xl border border-[#d4af37]/10 bg-[#16161d]" aria-label="جارٍ تحميل المنتجات" />}{data?.products.map((product, index) => {
      const hasVariants = product.variants.some(variant => variant.isActive);
      const selectableVariantStock = product.variants.filter(variant => variant.isActive).reduce((total, variant) => total + variant.stock, 0);
      const availableStock = hasVariants ? selectableVariantStock : product.stock;
      const shouldPrioritizeImage = index < 4;
      return <article key={product.id} className="group overflow-hidden rounded-xl border border-[#d4af37]/15 bg-[#0d0d12] transition duration-300 hover:-translate-y-1 hover:border-[#d4af37]/45"><Link href={`/products/${product.slug}`} className="block aspect-[4/3] overflow-hidden bg-black"><ShopCardImage url={primaryImage(product.images)} alt={product.name} prioritize={shouldPrioritizeImage} /></Link><div className="p-4 sm:p-5"><p className="text-xs font-semibold text-[#d4af37]/65">{product.category?.name || "إكسسوارات زجاج"}</p><Link href={`/products/${product.slug}`} className="mt-1 block min-h-12 text-lg font-black text-[#f5f0e8] transition hover:text-[#d4af37]">{product.name}</Link><p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-[#f5f0e8]/50">{product.description || "قطع مختارة لتركيبات الزجاج."}</p><div className="mt-5 flex items-center justify-between gap-3"><div><div className="flex flex-wrap items-baseline gap-2"><p className="text-xl font-black text-[#d4af37]">{formatMoney(product.priceAmount)}</p>{product.compareAtAmount && <p className="text-xs font-bold text-[#f5f0e8]/35 line-through">{formatMoney(product.compareAtAmount)}</p>}</div><p className={`mt-1 text-xs ${availableStock > 0 ? "text-emerald-400" : "text-rose-400"}`}>{availableStock > 0 ? (hasVariants ? "تتوفر تكوينات للاختيار" : `متوفر: ${availableStock}`) : "غير متوفر حاليًا"}</p></div>{hasVariants ? <Link href={`/products/${product.slug}`} className={`inline-flex min-h-10 items-center rounded-full px-4 text-sm font-bold ${availableStock > 0 ? "bg-[#d4af37] text-black hover:bg-[#f0d369]" : "pointer-events-none bg-white/10 text-white/35"}`}>اختر التكوين</Link> : <Button disabled={availableStock < 1} size="sm" className="min-h-10 rounded-full bg-[#d4af37] px-4 font-bold text-black hover:bg-[#f0d369]" onClick={() => addItem({ productId: product.id, variantId: null, variantLabel: null, slug: product.slug, name: product.name, priceAmount: product.priceAmount, imageUrl: primaryImage(product.images), stock: product.stock })}><ShoppingCart className="ml-1.5 h-4 w-4" />أضف</Button>}</div></div></article>;
    })}{!isLoading && data?.products.length === 0 && <div className="col-span-full min-h-44 rounded-xl border border-dashed border-[#d4af37]/30 bg-[#0d0d12] p-8 text-center sm:p-12"><h2 className="font-black text-[#f5f0e8]">لا توجد منتجات مطابقة</h2><p className="mt-2 text-sm text-[#f5f0e8]/50">جرّب كلمة بحث أو قسمًا مختلفًا.</p></div>}</div></div></main></StoreLayout>;
}
