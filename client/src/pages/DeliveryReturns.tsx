...
export default function DeliveryReturns() {
  ...
  return <StoreLayout><main>
    <StorePageHero eyebrow="معلومات الطلب" title={<>التوصيل والاستبدال <span className="text-gold-gradient">بوضوح</span></>} description="نفضّل تأكيد تفاصيل كل طلب قبل التنفيذ: المنتج، الكمية، عنوان التسليم، وتكلفة التوصيل إن وجدت. تواصل معنا إذا احتجت إلى مساعدة قبل الطلب.">
      ...
      {whatsappUrl && <a href={whatsappUrl} target="_blank" rel="noreferrer"><Button variant="outline" className="rounded-full border-[#d4af37]/45 text-[#d4af37] hover:bg-[#d4af37]/10 hover:text-[#f5f0e8]"><MessageCircle className="ml-2 h-4 w-4" />اسأل عبر واتساب</Button></a>}
    </StorePageHero>
    <section className="bg-[#08090d] py-14 sm:py-18">{isCatalogStaging && <div className="container mb-6"><div role="note" className="rounded-xl border border-amber-300/25 bg-amber-300/10 px-5 py-4 text-sm leading-7 text-amber-50">هذه معلومات إرشادية ضمن وضع العرض التجريبي. تفاصيل الأسعار، التوصيل، الاستبدال، الدفع، والالتزامات القانونية النهائية تنتظر اعتماد إدارة المتجر قبل تفعيل الطلبات.</div></div>}<div className="container grid gap-5 md:grid-cols-3">
    ...
    </div><div className="container mt-8"><div className="rounded-xl border border-[#d4af37]/20 bg-[#d4af37]/8 p-6 sm:p-8"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-3"><PackageCheck className="h-6 w-6 text-[#d4af37]" /><h2 className="text-xl font-black">هل تحتاج لتأكيد قبل الطلب؟</h2></div><p className="mt-2 max-w-2xl leading-7 text-[#f5f0e8]/60">أرسل لنا اسم القطعة أو رابط المنتج على واتساب، وسنساعدك في مراجعة توفرها وتفاصيلها.</p></div>{whatsappUrl && <a href={whatsappUrl} target="_blank" rel="noreferrer"><Button className="w-full rounded-full bg-[#25d366] font-black text-[#08090d] hover:bg-[#1fb85a] sm:w-auto">فتح واتساب</Button></a>}</div></div></div></section>
  </main></StoreLayout>;
}
