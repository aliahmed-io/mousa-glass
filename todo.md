...
- [ ] Address the remaining application-controlled published Lighthouse opportunities: variable initial-server response, approximately 36 KiB unused JavaScript, and the shared-stylesheet render-blocking insight. Fresh cache-busted Lighthouse 13.4.1 evidence at checkpoint `cb75f6b8` again found Home/Shop document response of 1,854/673 ms, 37,232/36,395 unused shared-entry bytes, and 100/80 ms estimated stylesheet savings; no low-risk application change was justified. Keep managed CSP/no-store reports and intentional staging crawl blocking documented rather than weakening safeguards; the latest measured Shop image-delivery overfetch is resolved.
...
- [ ] Perform a final live signed-in administrator passphrase smoke test after OAuth account selection. A 25 August 2026 non-invasive published-route recheck confirmed the expected Arabic sign-in gate and no existing administrative session; the full smoke test remains blocked because browser takeover and account selection were explicitly declined.
...
- [x] Add an Arabic public policy-status route that explicitly states privacy, sale, delivery, returns, COD, InstaPay-proof, and contact terms are pending qualified approval; do not present it as a legal policy or enable live ordering. The `/policy-status` route is source-tested, visually verified on desktop and mobile, and explicitly preserves staging-only ordering.
- [x] Correct the Arabic staging disclosure so it does not describe the configured contact channel as unapproved while retaining explicit generated-catalog, pricing, logistics, payment, legal, and real-order warnings. The corrected disclosure is regression-tested, readable on desktop and mobile, and full release validation reconfirmed staging mode and the real-order block.
