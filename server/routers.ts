...
import {
  getCategories, getCategoriesWithCount, createCategory, updateCategory, deleteCategory,
  getProducts, getProductById, getFeaturedProducts, getAllProducts,
  ...
  getDashboardStats, seedCategories, seedProducts, updateUserProfile,
} from "./db";

// Admin-only procedure
const adminProcedure = protectedProcedure.use(({ ctx, next }) => {
  if (ctx.user.role !== 'admin') throw new TRPCError({ code: 'FORBIDDEN', message: 'صلاحية المدير مطلوبة' });
  return next({ ctx });
});
...
export const appRouter = router({
  ...
  categories: router({
    ...
    create: adminProcedure.input(z.object({
      name: z.string().min(1, "اسم الفئة بالإنجليزية مطلوب"),
  ...
  orders: router({
    ...
    byId: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ ctx, input }) => {
      ...
      if (!order || ((order as any).userId !== ctx.user.id && ctx.user.role !== "admin")) {
    ...
    create: publicProcedure.input(z.object({
      ...
      shippingAddress: z.string().min(1, "عنوان التوصيل مطلوب"),
      city: z.string().optional(),
      notes: z.string().optional(),
    })).mutation(async ({ ctx, input }) => {
      const userId = ctx.user?.id ?? 0;

      // Auto-update user profile if authenticated
      if (ctx.user?.id && ctx.user.id > 0) {
        try {
          ...
          console.warn("[Auth] Failed to update user profile on order:", e);
        }
      }
